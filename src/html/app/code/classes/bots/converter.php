                ...
                switch (trim($c)) {
                    default:
                        case "1080p.mp4":
                            ...
                            if (!$file_out->exists()) {
                                \ffmpeg::convert($file_in, $file_out, 'ffmpeg -i "{{in}}" -vf scale=-2:1080 -c:v libx264 -profile:v main -pix_fmt yuv420p -c:a aac -ac 2 -movflags +faststart "{{out}}"');
                            }
                            break;
                        case "480p.mp4":
                            $file_out = new \File("/out/".$row["md5"].".480p.mp4");
                            if (!$file_out->exists()) {
                                \ffmpeg::convert($file_in, $file_out, 'ffmpeg -i "{{in}}" -vf scale=-2:480 -c:v libx264 -profile:v main -pix_fmt yuv420p -c:a aac -ac 2 -movflags +faststart "{{out}}"');
                            }
                            break;
                        case "240p.mp4":
                            $file_out = new \File("/out/".$row["md5"].".240p.mp4");
                            if (!$file_out->exists()) {
                                \ffmpeg::convert($file_in, $file_out, 'ffmpeg -i "{{in}}" -vf scale=-2:240 -c:v libx264 -profile:v main -pix_fmt yuv420p -c:a aac -ac 2 -movflags +faststart "{{out}}"');
                            }
                            break;
